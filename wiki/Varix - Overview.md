...
- [[Saint Michael POS - Overview]] — client build: multi-brand restaurant/hospitality POS, inventory, HR/payroll, and analytics platform (see also [[Saint Michael POS - Capabilities]] for the full module-by-module dossier)
...
## Base
Los Baños, Philippines — building for Southeast Asia and global scale.

## Contact
General inquiries: `hello@varixph.com` (used across the business/legal templates — see [[Business Documents]]) vs. `hello@varix.work` (used in the Company Profile and founder emails — see [[Founders]]). The live site config splits the same way (`url` on `varix.work`, `email` on `varixph.com`). Not yet reconciled — worth a deliberate decision.
